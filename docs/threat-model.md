...
| A14 | **Storage layer compromise** | An attacker gains direct access to the PostgreSQL database and modifies stored DSSE envelopes or metadata. | Stored records are altered at the storage level, bypassing VAOL server controls. | (1) Every stored record is a signed DSSE envelope; modifying the envelope payload invalidates the signature. (2) Persisted Merkle leaves are deterministically bound to `record_hash` (`leaf_hash = SHA-256(0x00 || record_hash)`) and validated at startup. (3) Startup validation checks tree size vs record count and checkpoint root/signature; divergence triggers fail-closed startup when configured. (4) Auditors verify records by checking DSSE signatures, record hashes, and Merkle proofs; database-level tampering is detected during verification. (5) Database access should be restricted to the VAOL server service account with minimal privileges. | If the attacker can manipulate both database state and signer keys, they may forge coherent state. Use KMS/HSM + external checkpoint anchoring to reduce that risk. |
...
4. **Merkle hot state remains in-memory.** VAOL now persists Merkle leaf hashes and restores tree state from `merkle_leaves` on startup, with fallback traversal rebuild. Large ledgers still incur restore-time memory and I/O cost proportional to tree size.
...
| Low | For very large deployments, add Merkle compaction/snapshot rotation to bound startup restore time and memory usage. | A10 |
